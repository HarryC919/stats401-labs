async function drawChart() {
    ...
    const xOuter = d3.scaleBand()
        ...
        .padding(0.2);
    const xInner = new Map(Array.from(grouped, ([regions, cities]) => [
        regions,
        d3.scaleBand()
    ...
    const developmentLevelScale = d3.scaleOrdinal()
        .domain([
        "Low",
        "Medium",
        "High"
    ...
    const developmentLevel = ["Low", "Medium", "High"];
    ...
    legendDevelopmentLevel.append("rect")
        .attr("height", 12)
        .attr("width", 12)
        .attr("stroke", "#333333")
        .attr("fill", "#FFFFFF")
        .attr("stroke-width", d => developmentLevelScale(d));
    legendDevelopmentLevel.append("text")
        .attr("x", 16)
        .attr("y", 12)
        .text(d => d);
    ...
    gradient.selectAll("stop")
        ...
        .attr("stop-color", d => temperatureScale(minTemp + (d / (stopCount - 1)) * (maxTemp - minTemp)));
    const legendTemperature = svg.append("g")
        .attr("transform", `translate(${width - margin.right + 25}, ${60 + 3 * 28 + 30})`);
    legendTemperature.append("text")
        .attr("y", -8)
        .attr("font-weight", "bold")
        .text("Temperature (°C)");
    legendTemperature.append("rect")
        .attr("width", 120)
        .attr("height", 12)
