...
async function drawChart(): Promise<void> {
    ...
    const [minTemp, maxTemp] = d3.extent(data, d => d.temp_c) as [number, number];
    
    const temperatureScale = d3.scaleSequential(d3.interpolateRdBu)
        .domain([maxTemp, minTemp]);

    const developmentLevelScale = d3.scaleOrdinal<string, number>()
    .domain([
        "Low",
        "Medium",
        "High"
    ...
    svg.selectAll(".city-label")
        ...
        .attr("x", d =>
            xOuter(d.region)! + xInner.get(d.region)!(d.city)!
